...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      const wikiRows = wiki.map((item) => ({ id: item.id, title: item.title, slug: item.slug, category: item.category, summary: item.summary, content: item.content, tags: item.tags, kit: item.kit ?? null, grade: item.grade ?? null, scale: item.scale ?? null, release: item.release ?? null, price: item.price ?? null, views: item.views, likes: item.likes, status: item.status, revision: item.revision, updated_at: item.updatedAt, image_url: item.imageUrl ?? null }));
  ...
  function updateCollection(pageId: number, status: CollectionStatus, targetPrice = "") {
    if (user.role === "guest") { setNotice("登录后即可建立个人模型清单。"); goToLogin(); return; }
