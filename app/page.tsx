...
type Section = "home" | "wiki" | "search" | "gallery" | "publish" | "forum" | "tools" | "collections" | "calendar" | "admin" | "profile" | "login" | "post-detail" | "forum-publish" | "work-detail" | "tool-detail";
type WikiPage = {
  id: number;
  title: string;
  slug: string;
...
type Post = { id: number; board: string; title: string; content: string; tags: string[]; author: string; replies: number; likes: number; pinned?: boolean; featured?: boolean; createdAt: string };
  ...
  return (
    <main className="min-h-screen bg-[#f5f7fb] text-slate-900">
      <div className="relative mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
        ...
        {section === "wiki" && selectedWiki && <WikiSection page={selectedWiki} pages={wiki} revisions={revisions.filter((item) => item.pageId === selectedWiki.id)} user={user} editing={editing} setEditing={setEditing} compare={compare} setCompare={setCompare} onSelect={openWiki} supabaseEnabled={supabaseEnabled} setNotice={setNotice} onTagSearch={searchByTag} collection={collections.find((item) => item.pageId === selectedWiki.id)} onCollection={updateCollection} tutorialProgress={tutorialProgress} onToggleStep={toggleTutorialStep} usefulVote={usefulVotes[selectedWiki.id]} onVoteUseful={voteUseful} onSave={(nextContent, summary, tags, imageUrl) => {
        ...
        {section === "forum" && <ForumSection posts={posts} user={user} openPost={openPost} setSection={setSection} onTagSearch={searchByTag} />}
        {section === "forum-publish" && <ForumPublishSection posts={posts} setPosts={setPosts} user={user} setNotice={setNotice} setSection={setSection} />}
...
function WikiSection({ page, pages, revisions, user, editing, setEditing, compare, setCompare, onSelect, onSave, onTagSearch, collection, onCollection, tutorialProgress, onToggleStep, usefulVote, onVoteUseful, supabaseEnabled, setNotice }: { page: WikiPage; pages: WikiPage[]; revisions: Revision[]; user: User; editing: boolean; setEditing: (v: boolean) => void; compare: boolean; setCompare: (v: boolean) => void; onSelect: (id: number) => void; onSave: (content: string, summary: string, tags: string[], imageUrl?: string) => void; onTagSearch: (tag: string) => void; collection?: CollectionItem; onCollection: (pageId: number, status: CollectionStatus, targetPrice?: string) => void; tutorialProgress: Record<string, boolean>; onToggleStep: (pageId: number, step: number) => void; usefulVote?: "useful" | "not-useful"; onVoteUseful: (pageId: number, vote: "useful" | "not-useful") => void; supabaseEnabled: boolean; setNotice: (notice: string) => void }) {
  ...
  return <section className={`wiki-page grid gap-6 lg:grid-cols-[300px_1fr] ${workbench ? "workbench-mode" : ""}`}>
    {!workbench && <aside className="space-y-4">
      ...
      <div className="rounded-[1.75rem] bg-white p-5 shadow-xl shadow-slate-200/60"><div className="mb-3 text-sm font-bold text-slate-400">知识库目录</div>{pages.map((item) => <button key={item.id} onClick={() => onSelect(item.id)} className={`mb-2 w-full rounded-2xl p-3 text-left transition ${item.id === page.id ? "bg-blue-600 text-white" : "bg-slate-50 hover:bg-blue-50"}`}><span className="block font-bold">{item.title}</span><span className="text-xs opacity-70">{item.category} · v{item.revision}</span></button>)}</div>
...
function ForumSection({ posts, user, openPost, setSection, onTagSearch }: { posts: Post[]; user: User; openPost: (id: number) => void; setSection: (section: Section) => void; onTagSearch: (tag: string) => void }) {
  ...
  return <section className="grid gap-6 lg:grid-cols-[280px_1fr]">
    <aside className="rounded-[1.75rem] bg-white p-5 shadow-xl"><div className="mb-5"><div className="text-xs font-bold tracking-[.2em] text-blue-600">COMMUNITY BOARD</div><h1 className="mt-1 text-2xl font-black">讨论版块</h1></div>{boards.map((item) => <button key={item} onClick={() => setBoard(item)} className={`mb-2 w-full rounded-2xl px-4 py-3 text-left font-bold ${board === item ? "bg-blue-600 text-white" : "bg-slate-50 text-slate-600"}`}>{item}<span className="float-right text-xs opacity-60">{posts.filter((post) => post.board === item).length}</span></button>)}</aside>
    <div className="space-y-5"><div className="flex flex-wrap items-center justify-between gap-4 rounded-[1.75rem] bg-white p-5 shadow-xl"><div><div className="text-xs font-bold tracking-[.2em] text-blue-600">SELECTED BOARD</div><h2 className="mt-1 text-3xl font-black">{board}</h2><p className="mt-1 text-sm text-slate-400">浏览话题或进入独立发布页发起新讨论。</p></div><button onClick={() => setSection(user.role === "guest" ? "login" : "forum-publish")} className="rounded-2xl bg-blue-600 px-6 py-3 font-bold text-white">{user.role === "guest" ? "登录后发布" : "＋ 发布新帖"}</button></div>
      {visiblePosts.map((post) => <article key={post.id} className="rounded-[1.75rem] bg-white p-5 shadow-xl transition hover:-translate-y-1"><button onClick={() => openPost(post.id)} className="block w-full text-left"><div className="flex flex-wrap gap-2">{post.pinned && <span className="rounded-full bg-red-50 px-3 py-1 text-xs font-bold text-red-600">置顶</span>}{post.featured && <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-600">精华</span>}<span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">{post.board}</span></div><h2 className="mt-3 text-xl font-black">{post.title}</h2><p className="mt-2 line-clamp-2 text-slate-600">{post.content}</p></button><div className="mt-4 flex flex-wrap gap-2">{(post.tags ?? []).map((tag) => <button key={tag} onClick={() => onTagSearch(tag)} className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">#{tag}</button>)}</div><div className="mt-4 text-sm text-slate-400">{post.author} · {post.replies} 回复 · {post.likes} 赞 · {post.createdAt}</div></article>)}
    </div>
  </section>;
}

function ForumPublishSection({ posts, setPosts, user, setNotice, setSection }: { posts: Post[]; setPosts: (posts: Post[]) => void; user: User; setNotice: (notice: string) => void; setSection: (section: Section) => void }) {
  ...
  function publish() {
    ...
    const nextPost: Post = { id: Date.now(), board, title: title.trim(), content: `【${topic.trim()}】\n${content.trim()}`, tags, author: user.nickname, replies: 0, likes: 0, createdAt: new Date().toISOString().slice(5, 10) };
    setPosts([nextPost, ...posts]);
    setNotice("帖子已发布到所选版块，并加入标签检索。");
    setSection("forum");
  }
  return <section className="mx-auto max-w-5xl space-y-5"><button onClick={() => setSection("forum")} className="font-bold text-blue-600">← 返回讨论区</button><div className="grid gap-6 lg:grid-cols-[1fr_300px]"><div className="rounded-[2rem] bg-white p-7 shadow-xl"><div className="text-xs font-bold tracking-[.22em] text-blue-600">CREATE DISCUSSION</div><h1 className="mt-2 text-4xl font-black">发布新帖</h1><p className="mt-2 text-slate-500">请填写明确标题、内容主题与正文，方便其他玩家快速理解和参与讨论。</p><div className="mt-7 space-y-5"><Field label="帖子标题 *" value={title} onChange={setTitle} /><Field label="内容主题 *" value={topic} onChange={setTopic} /><label className="block"><span className="text-sm font-bold text-slate-500">正文内容 *</span><textarea value={content} onChange={(event) => setContent(event.target.value)} placeholder="详细描述问题背景、已有尝试或希望讨论的方向，至少 10 个字。" className="mt-2 h-72 w-full rounded-2xl p-4" /><span className="mt-2 block text-right text-xs text-slate-400">{content.trim().length} 字</span></label></div></div><aside className="space-y-5"><div className="rounded-[1.75rem] bg-white p-5 shadow-xl"><h2 className="text-xl font-black">选择目标版块 *</h2><div className="mt-4 space-y-2">{boards.map((item) => <button key={item} onClick={() => setBoard(item)} className={`w-full rounded-xl px-3 py-2 text-left text-sm font-bold ${board === item ? "bg-blue-600 text-white" : "bg-slate-50 text-slate-600"}`}>{item}</button>)}</div></div><div className="rounded-[1.75rem] bg-white p-5 shadow-xl"><h2 className="text-xl font-black">自定义标签</h2><p className="mt-1 text-xs text-slate-400">使用中文或英文逗号分隔，最多保留 8 个标签。</p><input value={tagText} onChange={(event) => setTagText(event.target.value)} placeholder="例如：RG, 渗线, 新手求助" className="mt-4 w-full rounded-xl px-3 py-3" /><div className="mt-3 flex flex-wrap gap-2">{tags.map((tag) => <span key={tag} className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">#{tag}</span>)}</div></div><button disabled={!canPublish} onClick={publish} className="w-full rounded-2xl bg-blue-600 py-4 font-bold text-white disabled:bg-slate-300">确认发布到「{board}」</button></aside></div></section>;
}
...
function CollectionsSection({ collections, wiki, openWiki, onUpdate }: { collections: CollectionItem[]; wiki: WikiPage[]; openWiki: (id: number) => void; onUpdate: (pageId: number, status: CollectionStatus, targetPrice?: string) => void }) {
  const groups: { status: CollectionStatus; title: string; icon: string; desc: string }[] = [{ status: "wishlist", title: "想买清单", icon: "🛒", desc: "等待好价或再贩" }, { status: "planned", title: "想做清单", icon: "🧰", desc: "已入手，计划开工" }, { status: "completed", title: "已做清单", icon: "✅", desc: "完成制作的套件" }, { status: "abandoned", title: "弃坑清单", icon: "📦", desc: "暂停、转卖或放弃" }];
  return <section className="space-y-6"><div className="rounded-[2rem] bg-white p-7 shadow-xl"><div className="text-xs font-bold tracking-[.22em] text-blue-600">MY BACKLOG</div><h1 className="mt-2 text-4xl font-black">我的模型清单</h1><p className="mt-3 text-slate-500">管理想买、想做、已完成与弃坑套件，记录你的真实堆积与制作履历。</p><div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">{groups.map((group) => <div key={group.status} className="rounded-2xl bg-slate-50 p-4"><div className="text-2xl">{group.icon}</div><b className="mt-2 block">{group.title}</b><span className="text-2xl font-black text-blue-600">{collections.filter((item) => item.status === group.status).length}</span></div>)}</div></div><div className="grid gap-5 lg:grid-cols-2">{groups.map((group) => <article key={group.status} className="rounded-[1.75rem] bg-white p-5 shadow-xl"><div className="flex items-center justify-between"><div><h2 className="text-xl font-black">{group.icon} {group.title}</h2><p className="text-sm text-slate-400">{group.desc}</p></div></div><div className="mt-4 space-y-3">{collections.filter((item) => item.status === group.status).map((item) => { const page = wiki.find((entry) => entry.id === item.pageId); return page ? <div key={item.pageId} className="rounded-2xl bg-slate-50 p-4"><button onClick={() => openWiki(page.id)} className="block w-full text-left"><b>{page.title}</b><div className="mt-1 text-xs text-slate-400">{page.grade ?? "未分类"} · {page.scale ?? "比例未知"} · 加入于 {item.addedAt}</div>{item.targetPrice && <div className="mt-2 text-sm font-bold text-green-600">预期价格：{item.targetPrice}</div>}</button><select value={item.status} onChange={(e) => onUpdate(page.id, e.target.value as CollectionStatus, item.targetPrice)} className="mt-3 w-full rounded-xl px-3 py-2 text-sm">{groups.map((option) => <option key={option.status} value={option.status}>{option.title}</option>)}</select></div> : null; })}{!collections.some((item) => item.status === group.status) && <div className="rounded-2xl border border-dashed border-slate-200 p-6 text-center text-sm text-slate-400">从知识库条目页加入套件</div>}</div></article>)}</div></section>;
}
